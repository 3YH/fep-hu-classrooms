...
export class AanvraagService {
  private aanvraagCollection: AngularFirestoreCollection<Aanvraag> = this.angularFirestore.collection('aanvragen');
      ...
      .collection<Aanvraag>(
        ...
        (reference: Query) =>
          isNullOrUndefined(whereClause)
            ? reference
            : reference.where(whereClause.fieldPath, whereClause.operator, whereClause.value)
      ).valueChanges();
  }
  ...
  public getAanvraagById(aanvraagId: string): Observable<Aanvraag[]> {
    return this.getAanvragen({ fieldPath: 'aanvraagId', operator: '==', value: aanvraagId } as WhereClause);
  }
  ...
  public getAanvraagByUserUid(userUid: string): Observable<Aanvraag[]> {
    return this.getAanvragen({ fieldPath: 'aanvragerId', operator: '==', value: userUid } as WhereClause);
  }
   ...
   */
  public getAanvragenByUser(user: User | Docent | Student): Observable<Aanvraag[]> {
    if (isNullOrUndefined(user)) {
      console.log('Er is geen gebruiker opgegeven bij het zoeken naar aanvragen.');
      return of([]);
  ...
  public getAanvragenByDocent(docent: Docent): Observable<Aanvraag[]> {
    ...
    docent.beheerdeRuimtes.forEach((ruimteId: string) => observerList.push(this.getAanvragenByRuimteId(ruimteId)));
    return zip(...observerList)
      .pipe(
  ...
  public getAanvragenByRuimteId(ruimteId: string): Observable<Aanvraag[]> {
    return this.getAanvragen({ fieldPath: 'ruimteId', operator: '==', value: ruimteId } as WhereClause);
  }
  ...
  public getAanvragenByStatus(status: string): Observable<Aanvraag[]> {
    return this.getAanvragen({ fieldPath: 'status.aanvraagStatus', operator: '==', value: status } as WhereClause);
  }
  ...
  public async addAanvraag(aanvraag: Aanvraag): Promise<void> {
    const documentReference: DocumentReference = await this.aanvraagCollection.add(aanvraag);
